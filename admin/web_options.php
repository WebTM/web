...
class Web_Options {
	...
	public function __construct(){
		add_action('customize_register', 'web_additional_classes', 10 );
		...
		if( is_array($this->settings) ){
			foreach( $this->settings as $setting ){
				...
				if( 'image' == $type ){
					...
					$wp_customize->add_control( 
						new web_Customizer_Number_Control(
							$wp_customize, $name, array(
					    		'label'    => $title,
					    		'section'  => $section,
					    		'priority' => $priority
...
if(!( function_exists('web_additional_classes') )){
	function web_additional_classes(){
