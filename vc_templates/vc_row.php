...
if( 'image-left' == $background_style ){
	...
	$output .= '<section class="'. $background_style .' '.$css_class.'"'.$style.'><div class="row">';
	$output .= wpb_js_remove_wpautop($content);
	...
	$el_class = $this->getExtraClass($el_class);
	$el_class = str_ireplace( " ", "", $el_class );
	$css_class = apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, $el_class .' '. ( $this->settings('base')==='vc_row_inner' ? 'vc_inner ' : '' ) . get_row_css_class() .  vc_shortcode_custom_css_class( $css, ' ' ), $this->settings['base'], $atts );


	$style = $this->buildStyle($bg_image, $bg_color, $bg_image_repeat, $font_color, $padding, $margin_bottom);
	$output .= '<section class="'.$style.$css_class.'"><div class="container"><div class="row">';
	$output .= wpb_js_remove_wpautop($content);
